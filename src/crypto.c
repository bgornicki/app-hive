...
bool crypto_sign_message() {
    ...
    cx_hash((cx_hash_t *) &G_context.tx_info.sha, CX_LAST, G_context.tx_info.m_hash, 0, G_context.tx_info.m_hash, sizeof(G_context.tx_info.m_hash));

    PRINTF("Digest: %.*H\n", DIGEST_LEN, G_context.tx_info.m_hash);
    ...
    BEGIN_TRY {
        TRY {
            ...
            while (true) {
                if (counter == 0) {
                    rng_rfc6979(der_signature, G_context.tx_info.m_hash, private_key.d, private_key.d_len, SECP256K1_N, ARRAYLEN(SECP256K1_N), V, K);
                } else {
                    rng_rfc6979(der_signature, G_context.tx_info.m_hash, NULL, 0, SECP256K1_N, ARRAYLEN(SECP256K1_N), V, K);
                }
                ...
                if (!signature_from_der(der_signature, G_context.tx_info.signature, MEMBER_SIZE(transaction_ctx_t, signature))) {
                    is_valid = false;
                    break;
                }

                if (signature_check_canonical(G_context.tx_info.signature + 1)) {
                    break;
                } else {
                    counter++;
                }
